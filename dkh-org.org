...
#+begin_src org :exports src
  ...
  ,  [[file:~/git/src/org-mode-doc/org-mode.org::*Capture%20Tasks%20is%20all%20about%20being%20FAST][Capture Tasks is all about being FAST]]
#+end_src
...
When someone wants details of what I've done recently I simple
generate a log report in the agenda with tasks state changes combined
with a clock report for the appropriate time period.

The following setting shows task state changes in the agenda.
Combined with the agenda clock report ('R') I can quickly generate all
...
=C-c / t= combination leaving the buffer in a narrowed state.  I use
...
lines but the default =org-cycle-separator-lines= setting hides single
...
The following setting hides single blank lines and exposes the rest so
I can clean them up.  A single blank line between headings sometimes
reads nicer... so I allow _one_ blank line only.
...
but allows list items to adapt to existing blank line behaviour:
...
setting.  However now that I've used it for a long time it makes
the clocking history list much easier to use -- so reduce the 
threshold so the clock history list (=C-u C-F11=) shows the menu
on the right.
...
#+begin_src sh :exports none
  cd ~/git/foss/org-mode && git describe origin/master
#+end_src

This is version call_org-mode-doc-version() of this document and it is
created using the publishing features of =org-mode= git version call_org-mode-version().
